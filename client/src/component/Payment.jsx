  ...
  const handleCreditSubmit = (event) => {
    ...
    recurly.token(formRef.current, async (err, token) => {
      if (err) {
        ...
        // further actions
        if(subscriptionId){
          // If subscription is needed to be upgraded, then first terminate previous one, then create/add new subs, by following proper billing info mechanism
          ...
          try {
            // const res = await axios.post(
            //   `${import.meta.env.VITE_BACKEND_SERVER_URL}/subscription/${subscriptionId}/change`,
            //   changeSub
            // );

            const res = await axios.delete(
              `${import.meta.env.VITE_BACKEND_SERVER_URL}/subscription/${subscriptionId}`
            );
  
            console.log("Subscription deleted successfully!");
            console.log(res.data);

            toast.success('Previous Subscription is terminated successfully!');
            setTimeout(() => {
              setIsLoading(false);  
              // navigate('/membership');
            }, 2000);
            ...
            console.log("Error while terminating subscription:", error);

            // Check if there's a response and a message from the backend
            const errorMessage = error.response && error.response.data && error.response.data.message 
              ? `Error while subscribing: ${error.response.data.message}`
              : 'Error while subscribing.';

            // Display detailed error message in toast
            toast.error(errorMessage);
            toast.error("Cannot Change Plan - Please purchase a subscription.");
            setIsLoading(false);
            
            setTimeout(() => {
              navigate('/select-subscription', { state: {} });
            }, 4000); // 2 seconds delay to allow toast messages to display
                ...
                <Box p={3} border="1px" borderColor="gray.700" borderRadius="md" bg="gray.900">
                  <form id="payment-form" onSubmit={handleCreditSubmit} ref={formRef}>
                    ...
                    <Flex justifyContent="flex-end" mt={5}>
                      <Button bg="#C5B8EA" width={{ base: '100%', md: '250px' }} size="lg" type="submit" isDisabled={isLoading}>
                      {isLoading ? (
                        <><Spinner size="sm" mr={2} />
                        <Text as="span" fontWeight="bold" color="red.500">Processing Payment...</Text></>
